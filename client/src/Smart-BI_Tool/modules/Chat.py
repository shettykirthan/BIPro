...
def show_page():
    ...
    def select_chat(chat):
        ...
        # Load the CSV data for this chat
        ...
        if response.status_code == 200:
            ...
            for csv_file in csv_files:
                ...
                st.error(f"Could not find CSV file with ID: {chat['csv_id']}")
        else:
    ...
    with st.sidebar:
        ...
        try:
            ...
            # Display chat history with CSV file names
            for chat in st.session_state.chat_history:
                chat_title = chat.get("chat_title", "New Chat")
                csv_id = chat.get("csv_id")
                if csv_id:
                    csv_response = requests.get(GET_CSV_FILES_URL.format(user_id=user_id), headers=headers)
                    if csv_response.status_code == 200:
                        csv_files = csv_response.json().get("csvFiles", [])
                        for csv_file in csv_files:
                            if csv_file["_id"] == csv_id:
                                chat_title = f"{chat_title} ({csv_file['fileName']})"
                                break
    ...
    # File uploader (only show if no CSV is currently loaded)
